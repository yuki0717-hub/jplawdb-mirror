...
公式XML、本則の条文別テキスト、附則を `egov-law-db` に生成します。
...
- `egov-law-db/supplementary/{law_code}.txt`: 附則（条・項・号の構造付き）
...
主要な税法・施行令12種は、2025年4月1日、2024年4月1日、2023年10月1日時点の本文と附則も保存します。
さらに、e-Govが提供する法令履歴ID、施行日、改正法令名を含む改正履歴を保存します。

- `egov-law-db/history/index.html`: 過去基準日と改正履歴の一覧
- `egov-law-db/history/{YYYY-MM-DD}/text/{law_code}/{article}.txt`: 過去条文
- `egov-law-db/history/{YYYY-MM-DD}/supplementary/{law_code}.txt`: 過去附則
- `egov-law-db/history/revisions/{law_code}.json`: 改正履歴
- `egov-law-db/history/status.json`: 対象日、条文数、附則数、改正履歴数

AIへ質問する際は、取引日または事業年度開始日を明記し、その日以前で最も近い過去基準日を選ばせてください。
収録基準日と取引日が一致しない場合は、その間の改正履歴と公式原文を追加確認する必要があります。
e-Gov API Version 2の過去データ提供範囲は2017年4月1日以降です。

## 国税庁公式資料の直接同期

複雑な税務質問で使用する国税庁資料11件は、既存スナップショットとは別に国税庁公式サイトから毎回直接取得します。
UTF-8とShift_JISを判別して本文を抽出し、公式URL、ページタイトル、必須語、法令等現在日、取得日、Last-Modified、SHA-256を検証します。

- `nta-official-db/index.html`: 人向けの公式資料一覧
- `nta-official-db/status.json`: 取得日、法令等現在日、文字コード、ハッシュ
- `nta-official-db/text/{code}.txt`: AI向け公式資料本文
- `nta-official-db/raw/{code}.html.txt`: UTF-8変換後の公式HTML原文

必須資料を取得できない場合、内容が想定と異なる場合、法令等現在日が設定した上限より古い場合は公開しません。
同じ資料が旧データベースにもある場合は `nta-official-db` を優先してください。

## 複雑な税務質問の作動テスト

毎回のビルドで、役員給与、移転価格、仕入税額控除、収益認識、非上場株式評価、加算税に加え、過去法令を使う3件を含む9シナリオを検査します。
法律・施行令・過去法令・通達・国税庁解説など30資料について、ファイルの存在、UTF-8、必須語、参照リンクを確認します。
...
- e-Gov法令24種の法令ID・履歴ID・基準日・XML・附則を検証
- 過去法令3基準日、主要12法令、改正履歴の件数とハッシュを検証
...
- 分割された国税庁手引き索引
- 法令の枝番条文
- e-Gov XMLの法令名一致、本則・附則抽出、枝番号変換
- e-Gov現在法令・過去法令・改正履歴と生成ファイル数の整合
- latin_termsの相対パス
- ゼロバイトファイル
- 同一サイズの改ざん
- HTMLリンク切れ
